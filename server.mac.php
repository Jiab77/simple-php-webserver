...
{
    public static string $shortOptions = 'i::p::c::d::h::';
...
$config = new ConfigurationDTO(
    ...
    numberOfCores: $options['c'] ?? $options['cores'] ??= trim(shell_exec("sysctl -n hw.ncpu")),
    host: $options['i'] ?? $options['interface'] ??= '127.0.0.1',
    port: $options['p'] ?? $options['port'] ??= 8000,
    pathToPhp: trim(shell_exec("which php")),
);
...
function run_server(ConfigurationDTO $config): void
{
    ...
    pcntl_exec(
        ...
        $arguments,
        ['PHP_CLI_SERVER_WORKERS' => $config->numberOfCores - 1]
    );
}
